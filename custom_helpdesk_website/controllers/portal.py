...
class CustomerPortalInherit(portal.CustomerPortal):
    ...
    def _prepare_home_portal_valuess(self, counters):
        ...
        if 'ticket_count' in counters:
            ...
            #
            #     values['ticket_count'] = (
            #         request.env['helpdesk.ticket'].sudo().search_count([('team_id.equipe', '=', 'service_clientele')])
            #
            #         if request.env['helpdesk.ticket'].check_access_rights('read', raise_exception=False)
            #         else 0
            #     )
    ...
    def _prepare_helpdesk_tickets_domainn(self):
        ...
        return [('team_id.equipe', '=', 'support_rh')]
    ...
    def update_webticket(self, **kw):
        ...
        if request.session.uid:
            ...
            wecheck = False
            aucun_f = False
            personne_avec_droit_sap_copier = False
            if 'nomad' in kw:
                if kw['nomad']=='on':
                    nomad = True
            ...
            if 'personne_avec_droit_sap_copier' in kw:
                personne_avec_droit_sap_copier = int(kw['personne_avec_droit_sap_copier'])
            ...
            ticket.write({'stage_id': stage_id, 'collaborateur': collaborateur_id, 'personne_remplace': partner_id, 'pc_housse': kw['pc_housse'], 'clavier_souris': kw['clavier_souris'], 'x_studio_smartphone': kw['x_studio_smartphone'], 'x_studio_ligne_fixe': kw['x_studio_ligne_fixe'], 'x_studio_ligne': kw['x_studio_ligne'], 'nom_1': kw['nom_1'], 'nom_2': kw['nom_2'], 'tablette_chargeur_housse_clavier': kw['tablette_chargeur_housse_clavier'], 'nomad': nomad, 'start': start, 'aucun': aucun, 'etafi': etafi, 'kyriba': kiryba, 'wecheck': wecheck, 'aucun_f': aucun_f, 'logiciel_conge': kw['logiciel_conge'], 'logiciel_paye': kw['logiciel_paye'], 'logiciel_sap': kw['logiciel_sap'], 'nom': kw['nom'], 'personne_avec_droit_sap_copier': personne_avec_droit_sap_copier, 'logiciel_sap_analytic': kw['logiciel_sap_analytic'], 'justification': kw['justification'], 'vehicule': kw['vehicule'], 'personne_vehicule': kw['personne_vehicule'], 'acces_dossier': kw['acces_dossier'], 'nom_reseau': kw['nom_reseau'], 'besoin_epi': kw['besoin_epi'], 'comment_manager': kw['comment_manager']})
