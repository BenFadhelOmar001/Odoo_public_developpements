...
class InvoiceStampTax(models.Model):
    ...
    def default_get(self,default_fields):
        ...
        res = super(InvoiceStampTax, self).default_get(default_fields)

        context = self._context
        #_logger.info("****res.get('default_move_type') %s", context.get('default_move_type'))
        taxes_vente = self.env['account.tax'].search([('is_stamp', '=', True), ('type_tax_use', '=', 'sale')])
        taxes_achat = self.env['account.tax'].search([('is_stamp', '=', True), ('type_tax_use', '=', 'purchase')])


        if context.get('default_move_type') == 'in_invoice':
            #_logger.info("****facture fournisseur")

            res['stamp_tax'] = taxes_achat.id

        if context.get('default_move_type') == 'out_invoice':
            #_logger.info("****facture client")

            res['stamp_tax'] = taxes_vente.id
