...
class releve_partner_wiz(models.TransientModel):
    ...
    def generer_releve_report(self):
        ...
        return self.env.ref("releve_client_fournisseur.action_report_releve_partner_wiz").report_action(
            self, data=data
        )
...
class ReportRelevePartnerWiz(models.AbstractModel):
    ...
    _name = "report.releve_client_fournisseur.releve_partner_wiz_report"
    ...
    def total_amount_untaxed(self, record):
        date_from = record.date_debut
        date_to = record.date_fin
        partner_id = record.partner_id
        the_move_type = record.the_move_type
        ...
        if line_ids:
            ...
            for enreg in line_ids:
                if enreg.move_type == 'in_refund':

                    total_amount_untaxed = total_amount_untaxed + -(enreg.amount_untaxed)

                elif enreg.move_type == 'out_refund':
    ...
    def get_reglement_from_move(self, record,move):
        ...
        # Step 1: Get move lines of the invoice that affect the receivable/payable account
        invoice_lines = move.line_ids.filtered(lambda line: line.account_id.internal_type in ('receivable', 'payable'))

        # Step 2: Get all partial reconciliations from those lines
        partial_reconciles = invoice_lines.mapped('matched_debit_ids') + invoice_lines.mapped('matched_credit_ids')

        # Step 3: From reconciliations, get the payments (account.payment) related to this invoice
        payments = partial_reconciles.mapped('debit_move_id.payment_id') + partial_reconciles.mapped('credit_move_id.payment_id')

        # Filter out any empty records (if there are any)
        payment_ids = payments.filtered(lambda p: p)
        ...
        if payment_ids:
            for enreg in payment_ids:
                ...
                #reg = str(jour)+'/'+str(mois) +' '+ str(enreg.payment_method_line_id.name) +' '+ str(enreg.payment_reference) + "\n"
                reg = str(jour)+'/'+str(mois) +' '+ str(enreg.payment_method_line_id.name) +' '+ "\n"
                str_reglement = str_reglement + reg

        return str_reglement.replace("\n", "<br/>")
    ...
    def _get_report_values(self, docids, data=None):
        ...
        return {
            ...
            "total_amount_untaxed": self.total_amount_untaxed,
